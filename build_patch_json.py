...
class Slide2Patch():
    ...
    def cut_with_annotation(self, annotation_dir, sdpc_path, color_annotation):
        geojson_name = os.path.basename(sdpc_path).split(".")[0] + ".*"
        annotation_paths = glob.glob(os.path.join(annotation_dir, geojson_name))
        for annotation_path in annotation_paths:
            if annotation_path.split(".")[-1] == "geojson":
                print("processing {}!".format(annotation_path))
                with open(annotation_path, 'r', encoding='UTF-8') as f:
                    geojson_data = json.load(f)
                    coords, colors = self.getcoords_from_geojson(sdpc_path, geojson_data, color_annotation)
                    self.save_patch(sdpc_path, coords, colors)
                return None

        print("GeoJSON annotation of {} does not exist!".format(sdpc_path))
        return None
    ...
    def getcoords(self, data_path, label_dic, color_annotation):
        ...
        if color_annotation:
            ...
            return coords, None
    def getcoords_from_geojson(self, data_path, geojson_data, color_annotation):
        if data_path.split(".")[-1] == "sdpc":
            from sdpc.Sdpc import Sdpc
            slide = Sdpc(data_path)
        else:
        ...
        # GeoJSON stores features in 'features' array
        ...
        for feature in features:
            ...
            if geometry_type == 'Polygon':
                # Flatten nested list and extract the exterior ring (first element)
                Pointsx = [int(point[0]) for point in coordinates[0]]
                Pointsy = [int(point[1]) for point in coordinates[0]]
            elif geometry_type == 'MultiPolygon':
                # Handle MultiPolygon (we'll just take the first polygon)
                Pointsx = [int(point[0]) for point in coordinates[0][0]]
                Pointsy = [int(point[1]) for point in coordinates[0][0]]
            else:
                continue  # Only handling Polygon and MultiPolygon for now
            
            # Get color information from GeoJSON properties if available
            color = feature.get('properties', {}).get('stroke', None)
